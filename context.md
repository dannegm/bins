...
The plan is to sign a short-lived JWT containing `{ uuid }` using `VITE_SESSION_SECRET` via the `jose` library. The user copies the link `https://bins.hckr.mx/login?token=eyJ...` to the new device. On `/login`, the token is verified, the UUID is extracted, and it overwrites the `uuid` in localStorage.

`VITE_SESSION_SECRET` is a frontend env var and therefore visible in the bundle — this is acceptable for personal/self-hosted use.
