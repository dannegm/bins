...
const DIAGRAM_TYPES = new Set([
    ...
    'c4container', 'c4component', 'c4dynamic', 'block-beta', 'architecture-beta',
    'xychart-beta', 'sankey-beta', 'packet-beta',
]);

const KEYWORDS = new Set([
    'subgraph', 'end', 'loop', 'alt', 'else', 'opt', 'par', 'critical',
    'break', 'rect', 'note', 'over', 'left', 'right', 'of', 'participant',
    'actor', 'activate', 'deactivate', 'create', 'destroy', 'autonumber',
    'title', 'accTitle', 'accDescr', 'section', 'classDef', 'class', 'style',
    'linkStyle', 'click', 'callback', 'href', 'direction', 'namespace',
    'state', 'note', 'as', 'link',
]);

const DIRECTIONS = new Set(['TD', 'TB', 'LR', 'RL', 'BT']);

const ARROW_RE = /(-{1,2}(?:>>?|>|o|x|\.){0,2}->?|={1,2}>?|~{2}|\.{1,2}>?|-{1,3}|>{1,2}|<{1,2}|\|>?|\|\||\|\]|\[\/|\/\|)/;
